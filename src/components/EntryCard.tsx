...
export function EntryCard({ entry, onDelete, onEdit }: { entry: Entry; onDelete:(id:string)=>void; onEdit:(e:Entry)=>void }){
  return (
    <li className="group relative flex flex-col rounded-2xl border border-[var(--line)] bg-white p-5 transition min-h-[186px] hover:border-[#C45A2A]/20">
      <div className="flex items-start justify-between gap-2">
        <span className="inline-flex items-center gap-1.5 rounded-full bg-[#F2F0EB] border border-[var(--line)] px-2.5 py-1 text-[11px] font-bold tracking-[0.08em]">
      ...
      <div className="mt-auto pt-4 flex items-center justify-between">
        ...
        <span className="flex items-center gap-1">
          <button onClick={()=>onEdit(entry)} aria-label={`Edit ${entry.nickname}`} className="rounded-full border border-transparent hover:border-[var(--line)] hover:bg-[var(--paper)] p-1.5 text-[#6B6B74] hover:text-[#1A1A1E] focus-visible:ring-2 focus-visible:ring-[#C45A2A] transition">
          ...
          <button onClick={()=>onDelete(entry.id)} aria-label={`Delete ${entry.nickname}`} className="rounded-full border border-transparent hover:border-[var(--line)] hover:bg-[var(--paper)] p-1.5 text-[#6B6B74] hover:text-red-600 focus-visible:ring-2 focus-visible:ring-[#C45A2A] transition">
