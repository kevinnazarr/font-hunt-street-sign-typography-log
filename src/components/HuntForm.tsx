...
export function HuntForm({ count, editing, onAdd, onUpdate, onCancel }: { count:number; editing: Entry|null; onAdd:(e:Entry)=>void; onUpdate:(id:string,p:Partial<Entry>)=>void; onCancel:()=>void }){
  ...
  return (
    <section aria-labelledby="log-form-heading" className="mt-6 rounded-2xl border border-[#EAE8E3] bg-white shadow-[0_8px_24px_rgba(26,26,30,0.06)] p-5 sm:p-6">
      ...
      <form ref={formRef} onSubmit={handleSubmit} noValidate className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <label className="grid gap-1.5">
          ...
          <textarea value={mood} onChange={e=>setMood(e.target.value)} rows={2} placeholder="e.g. Warm, condensed, 1970s grocery nostalgia" className="rounded-xl border border-[#EAE8E3] bg-[#FDFCF8] px-3 py-2.5 text-[14px] outline-none focus:ring-2 focus:ring-[#C45A2A] resize-none"/>
        ...
        <label className="grid gap-1.5">
          ...
          <input type="date" value={date} onChange={e=>setDate(e.target.value)} max={todayISO()} aria-invalid={touched && !!errs.date} className="rounded-xl border border-[#EAE8E3] bg-[#FDFCF8] px-3 py-2.5 text-[14px] outline-none focus:ring-2 focus:ring-[#C45A2A]"/>
        ...
        <div className="sm:col-span-2 flex flex-wrap items-center gap-3 pt-1">
          ...
          {editing ? <button type="button" onClick={onCancel} className="rounded-full border border-[#EAE8E3] px-5 py-2.5 text-[13px] font-medium hover:bg-[#F2F0EB] transition">Cancel</button> : <span className="text-[12px] text-[#6B6B74]">{count===0?"Your first find starts the collection.":"Newest appears first."}</span>}
        </div>
      </form>
    </section>
  )
